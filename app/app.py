...
def formAddCarro():
    if request.method == 'POST':
        ...
        if (request.files['foto'] != ''):
            ...
            # Llamado la funcion que procesa la imagen
            nuevoNombreFile = recibeFoto(file)
            resultData = registrarCarro(
                marca, modelo, year, color, puertas, favorito, nuevoNombreFile, cilindraje, velocidad)
            if (resultData == 1):
...
def viewDetalleCarro(idCarro):
    msg = ''
    if request.method == 'GET':
        # Funcion que almacena los detalles del carro
    ...
    if request.method == 'POST':
        ...
        if (request.files['foto']):
            ...
            resultData = recibeActualizarCarro(
                marca, modelo, year, color, puertas, favorito, fotoForm, idCarro, cilindraje, velocidad)
        else:
            fotoCarro = None
            resultData = recibeActualizarCarro(
                marca, modelo, year, color, puertas, favorito, fotoCarro, idCarro, cilindraje, velocidad)
...
def eliminarCarro(idCarro='', nombreFoto=''):
    ...
    # C:\xampp\htdocs\localhost\Crud-con-FLASK-PYTHON-y-MySQL\app
...
def recibeFoto(file):
    ...
    upload_path = os.path.join(
        basepath, 'static/assets/fotos_carros', nuevoNombreFile)
